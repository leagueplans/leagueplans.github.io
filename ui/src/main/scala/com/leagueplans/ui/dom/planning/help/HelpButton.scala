package com.leagueplans.ui.dom.planning.help
...
object HelpButton {
  def apply(modal: Modal): L.Button =
    Button(_.handledAs(Some(modalContents())) --> modal).amend(
      L.cls(Styles.button),
      FontAwesome.icon(FreeRegular.faCircleQuestion).amend(L.svg.cls(Styles.icon)),
    )

  @js.native @JSImport("/styles/planning/help/helpButton.module.css", JSImport.Default)
