...
import com.leagueplans.ui.dom.common.{Button, FormOpener, Modal}
...
object AddItemButton {
  def apply(
    ...
    Button(
      _.handledWith(_.sample(effectObserverSignal).collectSome) --> createClickObserver(itemFuse, modal)
    ).amend(
      "Add items",
      L.disabled <-- effectObserverSignal.map(_.isEmpty)
    )

  private def createClickObserver(itemFuse: Fuse[Item], modal: Modal): Observer[Observer[AddItem]] =
